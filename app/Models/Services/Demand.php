...
class Demand extends Model{
	...
	public static $consTips = [
		...
		'min.required' => '请填写最低价格',
		'max.required' => '请填写最高价格',
	...
	public static function add($cols){
		...
		$todayRelease = self::where('uid', $user->id)->where('created_at', '>=', $todayTime)->where('status', '>=', 0)->get();
		...
		if($obj->save()){
			if($releaseScore > 0){
				UserScore::addScore($releaseScore, $user, __('发布需求'));
			}
